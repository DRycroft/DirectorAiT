...
const MyProfile = () => {
  ...
  const fetchMemberData = async () => {
    try {
      // Look up the board_members record linked to the current authenticated user
      const { data, error } = await supabase
        .from("board_members")
        .select("id, full_name, preferred_title, public_job_title, short_bio, public_company_affiliations, professional_qualifications, public_contact_email, publish_preferences, profile_completed_at, board_id")
        .eq("user_id", user!.id)
      ...
      if (!data) {
        toast({
          ...
          description: "You don't have any pending board member profiles to complete.",
          variant: "destructive",
        });
      ...
      setMember(data as BoardMemberRecord);
      ...
      const prefs = (data.publish_preferences as Record<string, boolean>) || {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update public fields on board_members
      ...
      toast({
        title: "Profile saved",
        description: "Your board member profile has been updated.",
      });
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 pb-12 px-4">
        <div className="max-w-3xl mx-auto">
          <Card>
            <CardHeader>
              <CardTitle>Your Board Member Profile</CardTitle>
              <CardDescription>
